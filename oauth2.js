...
const vpnCheck = require("../misc/vpnCheck");
  ...
  app.get(settings.api.client.oauth2.callbackpath, async (req, res) => {
    ...
    <link href="https://fonts.cdnfonts.com/css/whitney" >
    <title>Please wait...</title>

    <center>
      <h1 style="color: red"> please wait...</h1>
      <progress></progress>
      <img src="https://i.ibb.co/TTcwyJb/Discord-logo-png-7617.png" alt="Discord-logo-png-7617" border="0">
      <p style="color: red"><center>We are redirecting you to the dashboard this may take few seconds </center></p>
    </center>
  ...
  app.get(`/submitlogin`, async (req, res) => {
    ...
    if (json.ok == true) {
      ...
      if (userinfo.verified == true) {
          ...
          if (newsettings.api.client.allow.newusers == true) {
            ...
            if (await accountjson.status == 201) {
              ...
              if (user.length == 1) {
                ...
                return res.send("An error has occured when attempting to create your account plese alert admin.");
              };
            };
            log('signup', `${userinfo.username}#${userinfo.discriminator} logged in to the Alphactyl dashboard for the first time!`)
          } else {
        ...
        if (await cacheaccount.statusText == "Not Found") return res.send("An error has occured while attempting to get your information.");
      ...
      res.send("Please verify the email on your Discord account.");
    } else {
      res.redirect(`/login`);
    };
  });
